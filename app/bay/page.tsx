...
import MultiPartProgressBar, {
  ProgressSegment,
} from "@/components/common/MultiPartProgressBar";
...
import {
  ReviewModeProvider,
  useReviewMode,
} from "@/app/contexts/ReviewModeContext";
import ProjectFlagsEditor, {
  ProjectFlags,
} from "@/components/common/ProjectFlagsEditor";
...
async function editProjectAction(
  state: FormSave,
  formData: FormData
): Promise<FormSave> {
  const response = await fetch("/api/projects", {
...
function ProjectDetail({
    ...
    setProjects((prevProjects) =>
      prevProjects.map((p) =>
        p.projectID === project.projectID
          ? (updatedProjectData as ProjectType)
          : p
      )
    );
  };
  ...
  const [currentEditorFlags, setCurrentEditorFlags] =
    useState<ProjectFlags>(projectFlags);
      ...
      <div className="space-y-5 pb-8">
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          <div className="text-center text-sm">
            <p>
              This project contributes <strong>{projectHours}</strong> hour
              {projectHours !== 1 && "s"} (
              <strong>{contributionPercentage}%</strong>) toward your island
              journey
            </p>
            <ProjectStatus
            ...
            <div>
              ...
              <p className="text-lg font-semibold mt-1">
                {project.hackatimeLinks && project.hackatimeLinks.length > 0
                  ? `${project.hackatimeLinks.reduce(
                      (sum, link) => sum + (link.rawHours || 0),
                      0
                    )}h`
              ...
              <p className="text-lg font-semibold mt-1">
                {(() => {
                  if (
                    ...
                    const totalApproved = project.hackatimeLinks.reduce(
                      (sum, link) => {
                        return (
                          sum +
                          (link.hoursOverride !== null &&
                          link.hoursOverride !== undefined
                            ? link.hoursOverride
                            : 0)
                        );
                      },
                    ...
                    return project.hoursOverride !== undefined &&
        ...
        <ProjectReviewRequest
          ...
          onRequestSubmitted={(updatedProject, review) => {
            ...
            setProjects((prevProjects) =>
              prevProjects.map((p) =>
                p.projectID === project.projectID
                  ? ({ ...p, in_review: true } as ProjectType)
                  : p
              )
            );
        ...
        {!isReviewMode && (
          <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg">
            <h3 className="text-sm font-medium text-gray-700 mb-3 col-span-2">
              Project Status
            </h3>
            <div className="flex items-center">
              <div
                className={`w-3 h-3 rounded-full mr-2 ${
                  projectFlags.viral ? "bg-green-500" : "bg-gray-300"
                }`}
              ></div>
            ...
            <div className="flex items-center">
              <div
                className={`w-3 h-3 rounded-full mr-2 ${
                  projectFlags.shipped ? "bg-green-500" : "bg-gray-300"
                }`}
              ></div>
            ...
            <div className="flex items-center">
              <div
                className={`w-3 h-3 rounded-full mr-2 ${
                  projectFlags.in_review ? "bg-green-500" : "bg-gray-300"
                }`}
              ></div>
...
export function BayWithReviewMode({
  session,
  status,
  router,
  impersonationData,
  ...
  const startAnimation = useCallback(
    (targetCurrency: number, targetPercentage: number) => {
      ...
      const animateValue = (currentTime: number) => {
        ...
        const currentCurrencyValue = Math.floor(
          startCurrency + (targetCurrency - startCurrency) * easedProgress
        );
        const currentPercentageValue = Math.round(
    ...
    async function loadHackatimeProjects() {
      try {
        ...
        const hours = Object.fromEntries(
          projects.map((project: HackatimeProject) => [
            project.name,
            project.hours || 0,
          ])
        );
  ...
  const [projectCreateState, projectCreateFormAction, projectCreatePending] =
    useActionState(
      (state: FormSave, payload: FormData) =>
        new Promise<FormSave>((resolve, reject) => {
  ...
  const [projectEditState, projectEditFormAction, projectEditPending] =
    useActionState(
      (state: FormSave, payload: FormData) =>
        new Promise<FormSave>((resolve, reject) => {
          toast.promise(editProjectAction(state, payload), {
            ...
            success: (data) => {
              ...
              setProjects((prev) =>
                prev.map((p) =>
                  p.projectID === (data.data as ProjectType).projectID
  ...
  const calculateProgressSegments = (): ProgressSegment[] => {
    ...
    if (metrics.shippedHours > 0) {
      segments.push({
        ...
        color: "#10b981", // Green
        label: "Shipped",
        tooltip: `${metrics.shippedHours.toFixed(
          1
        )} hours from shipped projects`,
        animated: false,
        status: "completed",
      });
    }
    ...
    if (metrics.otherHours > 0) {
      segments.push({
        ...
        color: "#3b82f6", // Blue
        label: "In Progress",
        tooltip: `${metrics.otherHours.toFixed(
          1
        )} hours from in-progress projects`,
        animated: true,
        status: "in-progress",
      });
    }
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm text-yellow-700">
                  ...
                  {impersonationData.user.name ||
                    impersonationData.user.email ||
                    "Unknown User"}
                  's bay page
                </p>
              </div>
      ...
      <div className={styles.progressSection}>
        <h2 className="text-2xl font-bold text-center text-gray-800 mb-4 mt-2.5 md:mt-1">
          {impersonationData
            ? `${
                impersonationData.user.name
                  ? `${impersonationData.user.name}'s Voyage`
                  : "User Voyage"
              }`
            : "Your Voyage"}
        </h2>
        <div className="border border-gray-300 rounded-lg p-3 sm:p-4 bg-white max-w-xl mx-auto">
          <div className="flex items-center justify-between w-full py-1 md:py-2">
            <div className="flex-grow px-2 sm:px-4 md:px-0">
              <div className="flex items-center justify-center gap-2 sm:gap-3 min-w-0">
                <Tooltip
                  content={`You've built ${projects.length} project${
                    projects.length !== 1 ? "s" : ""
                  }, and grinded ${progressMetrics.rawHours} hour${
                    progressMetrics.rawHours !== 1 ? "s" : ""
                  } thus far`}
                >
                  <img
                    src="/ship2.png"
                ...
                <Tooltip content="Your prize - a fantastic island adventure with friends">
                  <img
                    src="/island2.png"
                    alt="Island"
                    className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 flex-shrink-0 flex items-center"
                  />
                </Tooltip>
              </div>
            </div>
          ...
          <div className="flex items-center justify-center gap-3 sm:gap-4 md:gap-6 mt-4">
            ...
            <Tooltip
              content={`Believe it or not, you are ${Math.round(
                progressMetrics.totalPercentage
              ...
              <div className="flex flex-col items-center justify-center w-24 sm:w-28 md:w-32 bg-gray-100 rounded-lg p-2 sm:p-3 md:p-4 h-[90px] sm:h-[100px] md:h-[108px]">
                <div
                  className={`text-xl sm:text-2xl md:text-3xl font-bold text-gray-700 ${
                    isGlowing ? "percentage-animated" : ""
                  }`}
                >
                  {animatedPercentage}%
                </div>
                <div
                  className={`text-[10px] sm:text-xs text-gray-500 text-center ${
                    isGlowing ? "percentage-animated" : ""
                  }`}
                >
                  Island Progress
            ...
            <Tooltip content="Approved & shipped hours that exceed 15h per project are converted into Clamshells.  These funds can be applied to travel costs, and also potentially converted to grantable prizes!">
              <div
                className="relative flex items-center justify-center bg-gray-100 rounded-lg p-2 sm:p-3 md:p-4 w-24 sm:w-28 md:w-32 h-[90px] sm:h-[100px] md:h-[108px]"
              ...
              >
                <div className="relative w-full h-full flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <span
                      className="font-bold text-sm sm:text-base text-center text-black"
                      style={{ marginTop: "-2px" }}
                    >
                      {animatedClamshells}
                    </span>
      ...
      >
        <div className="p-4">
          <h3 className="text-lg font-semibold mb-3">
            Your Journey to Shipwrecked
          </h3>
          <p className="mb-4">
            The progress bar shows your completion percentage towards the
            60-hour goal required to qualify for Shipwrecked.
          </p>
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-4">
            ...
            <ul className="list-disc pl-5 space-y-2">
              <li>
                ...
                ></span>
                <strong>Green:</strong> Hours from shipped projects (projects
                marked as "shipped")
              </li>
              <li>
                <span
              ...
              <li>
                ...
                ></span>
                <strong>Blue:</strong> Hours from in-progress projects (not yet
                shipped or viral)
              </li>
              <li>
                <span
            ...
            <p className="mt-3 text-sm text-gray-600">
              Hover over each segment in the progress bar to see the exact hours
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-4">
            ...
            <ol className="list-decimal pl-5 space-y-2">
              <li>
                Complete at least 60 hours of development time (roughly 15 hours
                per project) and ship 4 fully deployed projects
              </li>
              <li>
                Make at least one of your projects go viral according to our{" "}
                <a
                  href="/info/go-viral"
          ...
          <div className="mt-2 md:mt-6 w-full">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-2xl font-bold">
                {impersonationData
                  ? `${impersonationData.user.name || "User"}'s Projects`
                  : "Your Projects"}
              </h2>
              {!impersonationData && (
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-blue-700">
                      <strong>Review Mode Active:</strong> You can now add and
                      delete reviews on projects.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="text-sm text-gray-500 mb-2">
              <p className="hidden md:block">
                Click a project to select it. Press{" "}
                <kbd className="px-1 py-0.5 bg-gray-100 border border-gray-300 rounded">
                  Esc
                </kbd>{" "}
                to close.
              </p>
              <p className="md:hidden">Tap a project to view details.</p>
            </div>
              ...
              {projects
                ...
                .map((project, index) => (
                  <Project
                    ...
                    selected={
                      !isMobile && selectedProjectId === project.projectID
                    }
                  />
                ))}
              {projects.length === 0 && (
          ...
          <>
            {isProjectEditModalOpen ? (
              ...
              <div className={`${styles.editForm} relative`}>
                ...
                <form
                  action={projectEditFormAction}
                  className="overflow-y-scroll max-h-[95%]"
                >
                  <span className="invisible h-0 w-0 overflow-hidden [&_*]:invisible [&_*]:h-0 [&_*]:w-0 [&_*]:overflow-hidden">
                    ...
                    <FormInput
                      ...
                      placeholder="shipped"
                      defaultValue={
                        initialEditState.shipped?.toString() || "false"
                      }
                    >
                      {""}
                    </FormInput>
                    <FormInput
                      ...
                      placeholder="viral"
                      defaultValue={
                        initialEditState.viral?.toString() || "false"
                      }
                    >
                      {""}
                    </FormInput>
                    <FormInput
                      ...
                      defaultValue={
                        initialEditState.in_review?.toString() || "false"
                      }
                    >
                      {""}
                    </FormInput>
                  ...
                  <div className="mb-5 bg-gray-50 p-4 rounded-lg flex flex-wrap gap-2">
                    <label className="flex items-center text-sm text-gray-600 mr-4 cursor-not-allowed">
                      <input
                        type="checkbox"
                        checked={!!initialEditState.shipped}
                        readOnly
                        disabled
                      />{" "}
                      Shipped
                    ...
                    <label className="flex items-center text-sm text-gray-600 mr-4 cursor-not-allowed">
                      <input
                        type="checkbox"
                        checked={!!initialEditState.viral}
                        readOnly
                        disabled
                      />{" "}
                      Viral
                    ...
                    <label className="flex items-center text-sm text-gray-600 mr-4 cursor-not-allowed">
                      <input
                        type="checkbox"
                        checked={!!initialEditState.in_review}
                        readOnly
                        disabled
                      />{" "}
                      In Review
                  ...
                  {/* Debug info */}
                  <div
                    className="mb-5 p-3 border border-gray-200 rounded-lg text-xs text-gray-500"
                    style={{ display: "none" }}
                  >
                    <pre>
                      {JSON.stringify(
                  ...
                  <ProjectDetail
                    ...
                    onEdit={
                      impersonationData
                        ? () => {
                            toast.error(
                              "Cannot edit projects while impersonating users"
                            );
                          }
                        : (project) => {
        ...
        >
          {(() => {
            ...
            const getSelectedProjectHours = () => {
              ...
              if (
                !top4Projects.find(
                  (x) => x.project.projectID === selectedProject.projectID
                )
              )
                return 0;
              ...
              const hours =
                typeof selectedProject?.hoursOverride === "number" &&
                selectedProject.hoursOverride !== null
            ...
            const contributionPercentage = Math.round(
              (selectedProjectContribution / 60) * 100
            );
                ...
                {isReviewMode && (
                  <div className="bg-blue-50 border-l-4 border-blue-500 p-3 mb-4 rounded-r">
                    <div className="flex items-center">
                      ...
                      <div className="ml-3">
                        <p className="text-sm text-blue-700">
                          <strong>Review Mode Active:</strong> You can now add
                          and delete reviews on this project.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <div className="text-center text-sm">
                      <p>
                        ...
                        <strong>{selectedProjectContribution}</strong> hour
                        {selectedProjectContribution !== 1 && "s"} (
                        <strong>{contributionPercentage}%</strong>) toward your
                        island journey
                      </p>
                      <ProjectStatus
                        viral={selectedProject.viral}
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg mb-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-lg font-semibold mt-1">
                          {selectedProject.hackatimeLinks &&
                          selectedProject.hackatimeLinks.length > 0
                            ? `${selectedProject.hackatimeLinks.reduce(
                                (sum, link) => sum + (link.rawHours || 0),
                                0
                              )}h`
                        ...
                        <p className="text-lg font-semibold mt-1">
                          {(() => {
                            if (
                              ...
                              return selectedProject.hoursOverride !==
                                undefined &&
                                selectedProject.hoursOverride !== null
                  ...
                  <ProjectReviewRequest
                    ...
                    onRequestSubmitted={
                      impersonationData
                        ? () => {
                            toast.error(
                              "Cannot submit projects for review while impersonating users"
                            );
                          }
                  ...
                  <ProjectFlagsEditor
                    ...
                    onChange={
                      impersonationData
                        ? () => {
                            toast.error(
                              "Cannot modify project flags while impersonating users"
                            );
                          }
                        : (flags: ProjectFlags) => {
...
function ProjectModal(props: ProjectModalProps): ReactElement {
  ...
  const [isDeleteConfirmModalOpen, setIsDeleteConfirmModalOpen] =
    useState<boolean>(false);
  const [selectedHackatimeProjects, setSelectedHackatimeProjects] = useState<
    string[]
  >([]);
  ...
  const availableHackatimeProjects = useMemo(() => {
    ...
    Object.entries(props.hackatimeProjects).forEach(([label, projectName]) => {
      if (
        !usedHackatimeProjects.includes(projectName) &&
        projectName !== "<<LAST_PROJECT>>"
      ) {
  ...
  const handleDeleteConfirm = () => {
    ...
    // Show message that deletion is restricted
    toast.error(
      "Sorry, you cannot unlink your hackatime project from Shipwrecked."
    );
  };
            ...
            <>
              <div className="mb-5 bg-gray-50 p-4 rounded-lg">
                ...
                <FormInput
                  ...
                  state={props.state}
                  {...(props.playableUrl && {
                    defaultValue: props.playableUrl,
                  })}
                >
                  Playable URL
                </FormInput>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      Allow others to discuss this project
                    </p>
                  </div>
                  <input
                    type="checkbox"
...
function HackatimeMultiSelect({
  ...
  const unselectedProjects = availableEntries.filter(
    ([_, projectName]) => !selectedProjects.includes(projectName)
  );
