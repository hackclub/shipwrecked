...
interface ReviewSectionProps {
  projectID: string;
  userId: string; // The ID of the user who owns the project
...
export default function ReviewSection({
    ...
    if (hackatimeLinks && hackatimeLinks.length > 0) {
      hackatimeLinks.forEach((link) => {
        linkOverrides[link.id] =
          link.hoursOverride === null ? undefined : link.hoursOverride;
      });
    }
  ...
  const getFlagChangesDescription = (): string => {
    ...
    if (currentFlags.shipped !== originalFlags.shipped) {
      changes.push(
        `Shipped: ${originalFlags.shipped ? "Yes" : "No"} → ${
          currentFlags.shipped ? "Yes" : "No"
        }`
      );
    }
    ...
    if (currentFlags.viral !== originalFlags.viral) {
      ...
      changes.push(
        `Viral: ${originalFlags.viral ? "Yes" : "No"} → ${
          currentFlags.viral ? "Yes" : "No"
        }`
      );
    }
    ...
    hackatimeLinks.forEach((link) => {
      ...
      if (isValueDifferent) {
        ...
        const originalDisplay =
          originalValue !== undefined ? `${originalValue}h` : "none";
        const currentDisplay =
          currentValue !== undefined ? `${currentValue}h` : "none";

        console.log(
          `[DEBUG] Found override change: ${link.hackatimeName}: ${originalDisplay} → ${currentDisplay}`
        );
        linkChanges.push(
          `${link.hackatimeName}: ${originalDisplay} → ${currentDisplay}`
        );
      }
    });
    ...
    if (linkChanges.length > 0) {
      console.log(
        `[DEBUG] Found ${linkChanges.length} link changes:`,
        linkChanges
      );
    ...
    if (currentFlags.in_review !== originalFlags.in_review) {
      changes.push(
        `In Review: ${originalFlags.in_review ? "Yes" : "No"} → ${
          currentFlags.in_review ? "Yes" : "No"
        }`
      );
    }
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    if (
      (reviewResult === "reject" || reviewResult === "comment") &&
      !newComment.trim()
    ) {
    ...
    try {
      ...
      const hasOverrideChanges = Array.from(allKeys).some((key) => {
        const original =
          originalOverrides[key] === null ? undefined : originalOverrides[key];
        const updated =
          updatedOverrides[key] === null ? undefined : updatedOverrides[key];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b pb-2">
        ...
        {isReviewMode && (
          <div className="w-full sm:w-80">
            <HackatimeLanguageStats userId={userId} />
          </div>
        )}
      </div>
      ...
      {isReviewMode && initialFlags && (
        <div className="mb-6">
          ...
          <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md text-sm text-yellow-700">
            <p>
              Add a review comment and click <strong>Submit Review</strong>{" "}
              below to save these changes.
            </p>
          </div>
        </div>
      )}
        ...
        <form onSubmit={handleSubmitReview} className="mb-6">
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                <Icon
                  ...
                  size={16}
                  className={
                    reviewResult === "reject" ? "text-red-600" : "text-gray-500"
                  }
                />
                <span>Reject</span>
              </button>
            ...
            {reviewResult === "reject" && (
              <p className="mt-1 text-xs text-red-600">
                A comment explaining the rejection reason is required.
              </p>
            )}
            {reviewResult === "comment" && (
          ...
          >
            {isLoading
              ? "Submitting..."
              : flagsChanged
              ? "Submit Review with Flag Changes"
              : "Submit Review"}
          </button>
          ...
          <div className="mt-2 text-xs text-blue-600">
            ...
            <pre className="mt-1 p-2 bg-gray-50 rounded text-xs whitespace-pre-wrap">
              ...
              {(() => {
                ...
                // Check hour changes
                const origOverrides =
                  originalFlags.hackatimeLinkOverrides || {};
                ...
                hackatimeLinks.forEach((link) => {
                  ...
                  if (origValue !== currValue) {
                    hourChanges.push(
                      `${link.hackatimeName}: ${
                        origValue !== undefined ? origValue + "h" : "none"
                      } → ${currValue !== undefined ? currValue + "h" : "none"}`
                    );
                  }
                });
      ...
      <div className="space-y-4">
        {isFetchingReviews ? (
          ...
          <p className="text-gray-500 text-center py-4">
            No reviews yet.{" "}
            {isReviewMode ? "Be the first to review this project!" : ""}
          </p>
        ) : (
          <div className="space-y-4">
            {reviews.map((review) => (
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {isReviewMode &&
                          isCurrentUserReviewer(review.reviewerId) && (
                            <>
