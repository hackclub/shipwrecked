...
// Whitelist of users who can change shop items
const SHOP_ITEM_ADMIN_WHITELIST = (process.env.SHOP_ITEM_ADMIN_WHITELIST || '')
  .split(',')
  .map(e => e.trim())
  .filter(Boolean);
...
/**
 * Verifies that the current user is authenticated and whitelisted for shop item admin access
 * @returns Promise<ShopAdminAuthResult> - Success with user data or error with details
 */
export async function verifyShopItemAdminAccess(): Promise<ShopAdminAuthResult> {
  try {
    // Check authentication
    const session = await getServerSession(opts);
    if (!session?.user?.email) {
    ...
    // Check if user is in shop item admin whitelist
    if (!SHOP_ITEM_ADMIN_WHITELIST.includes(user.email)) {
      return {
        success: false,
    ...
    console.error('Shop item admin auth error:', error);
    return {
      success: false,
      error: 'Internal server error',
      status: 500
...
/**
 * Checks if an email is in the shop item admin whitelist
 * @param email - Email to check
 * @returns boolean - True if email is whitelisted
 */
export function isShopItemAdminWhitelisted(email: string): boolean {
  return SHOP_ITEM_ADMIN_WHITELIST.includes(email);
}

/**
 * Gets the list of whitelisted shop item admin emails (for debugging/testing)
 * @returns string[] - Array of whitelisted emails
 */
export function getShopItemAdminWhitelist(): string[] {
  return [...SHOP_ITEM_ADMIN_WHITELIST];
} 